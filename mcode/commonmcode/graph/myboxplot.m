...
%       'whisklength',size    : whisker length relative to box
...
while(c<=length(varargin))
    switch(varargin{c})
    case {'gap','CAT','linscale','split','subset','leg','leglocation'}
    ...
    case 'style_twoblock'
        ...
        CAT.fillcolor={[0.5 0.5 0.5],[0.8 0.8 0.8]};
        CAT.linecolor={[0.5 0.5 0.5],[0.8 0.8 0.8]};
        c=c+1;
    case 'style_block'
        F.plotall=2;
        F.linecolor=[0.8 0.8 0.8];
        ...
        F.markersize=5;
        % depending on the split-category membership
        CAT.fillcolor={[1 1 1],[0.6 0.6 0.6],[0 0 0]};
        CAT.linecolor={[0 0 0],[0.6 0.6 0.6],[1 1 1]};
        CAT.markerfill={[0.6 0.6 0.6],[0 0 0]};
        CAT.markercolor={[0.6 0.6 0.6],[0 0 0]};
        CAT.mediancolor={[0 0 0],[1 1 1],[1 1 1]};
        c=c+1;
    case 'style_tukey'
        F.plotall=1;
        F.linecolor=[0.8 0.8 0.8];
...
if (F.linscale ==0 )
    ...
    F.w = max(x_coord)*min(0.15,0.5/max(x_coord)); %boxwidth
else 
    x_coord=[D{:,1}];
...
for c=1:num_cat
    if (numsplitvars>0 & ~isempty(CAT))
        ...
        for f=1:length(fields)
            ...
            F=setfield(F,fields{f},fiel{indxspcat});
        end;
    end;
    if (sum(~isnan(D{c,2}))>0)
        [dummy,h(c)]=myboxutil(D{c,2}(~isnan(D{c,2})),x_coord(c),F);
    end;
end;
...
for j=1:num_cat
    if (~F.flip) 
        ...
        ht = text(xlim(1),x_coord(j),glabels{j,1},'HorizontalAlignment','center',...
            'VerticalAlignment','top', 'UserData','ytick');    end;
end
